...
const CreateQuizContent = () => {
    ...
    const dashboardPath = isTeacherDashboard
        ? "/dashboard/teacher/assessments?tab=quizzes"
        : "/dashboard/admin/assessments?tab=quizzes";
    const courseContentPath = (courseId: string) =>
        isTeacherDashboard
            ? `/dashboard/teacher/courses/${courseId}?tab=content`
            : `/dashboard/admin/courses/${courseId}?tab=content`;
    const goBackAfterQuizFlow = () => {
        if (selectedCourse) {
            router.push(courseContentPath(selectedCourse));
            return;
        }
        router.push(isTeacherDashboard ? "/dashboard/teacher/courses" : "/dashboard/admin/courses");
    };
