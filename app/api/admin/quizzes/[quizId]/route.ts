...
export async function GET(
    ...
    try {
        ...
        // Get the quiz; admin/teacher can access all quizzes
        const quiz = await db.quiz.findFirst({
            where: user?.role === "ADMIN" || user?.role === "TEACHER" || user?.role === "ADMIN_ASSISTANT"
...
export async function PATCH(
    ...
    try {
        ...
        // Only owner or admin/teacher can modify
        if (user?.role !== "ADMIN" && user?.role !== "TEACHER" && user?.role !== "ADMIN_ASSISTANT" && currentQuiz.course.userId !== userId) {
